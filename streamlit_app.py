...
#title
st.title("⏱ NATHEM FOODMART  DASHBOARD")
...
#load dataset
df = pd.read_excel("nathem_foodmart.xlsx",sheet_name="NathemFoodmart")
...
col2.metric(label="⏱ Sum of Product Total Price NGN:", value= f"{df_selection.TotalPrice.sum():,.0f}",delta=df_selection.TotalPrice.median())
col3.metric(label="⏱ Maximum Price  NGN:", value= f"{ df_selection.TotalPrice.max():,.0f}",delta="High Price")
col4.metric(label="⏱ Minimum Price  NGN:", value= f"{ df_selection.TotalPrice.min():,.0f}",delta="Low Price")
